...
| `templating_paths` | <code>["config/deploy/templates/#{fetch(:stage)}/%&lt;host&gt;s","config/deploy/templates/#{fetch(:stage)}", "config/deploy/templates/shared/%&lt;host&gt;s","config/deploy/templates/shared"]</code>| Folder to look for a template to render. `<host>` is replaced by the actual host. |
