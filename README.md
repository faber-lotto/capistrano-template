 ...
 ```ruby
   ...
   task :setup do     
     ...
     on roles :all       
       ...
       # owner is changed to "deployer"
       template 'other.template.name', '~/execute_some_thing.sh', 0750, 'deployer', locals: { 'local1' => 'value local 1'}
     end
...
|`templating_owner_test_cmd` | <code>%Q{ &#91; "Z$(stat -c "%%U" %&lt;path&gt;s 2&gt;/dev/null)" != "Z%&lt;owner&gt;s" &#93; }</code> | Test command to check the remote file permissions. |
